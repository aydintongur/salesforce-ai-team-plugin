...
4. Deploy with `sf project deploy start --source-dir <path> --target-org interview-practice`, then run the tests with `sf apex run test --target-org interview-practice --synchronous` and read the actual pass/fail output.
